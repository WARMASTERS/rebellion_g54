...
RSpec.describe RebellionG54::Action::Guerrilla do
  let(:user) { game.choice_names.keys.first }
  let(:opponent) { game.users.last }

  context 'when using guerrilla' do
    let(:game) { example_game(2, coins: 4, roles: :guerrilla) }
    before(:each) { game.take_choice(user, 'guerrilla', opponent) }
